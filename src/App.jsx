...
function App() {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      const joined = await joinRoom({ room, user })
      setRoom(joined)
      setScreen('cinema')
      setElapsed(0)
    } catch (error) { setRoomError(error.message || 'Could not join that cinema.') }
    finally { setRoomBusy(false) }
  }
  ...
  if (screen === 'cinema') {
    const friends = participants.filter((participant) => participant.uid !== user.uid)
    return <main className={`cinema-page mode-${mode.toLowerCase()}`}>
      ...
      <section className="cinema-stage"><div className="film-meta"><span className="provider-pill">TEST VIDEO</span><b>{film.title}</b><small>{film.detail}</small></div><div className="screen-wrap"><div className="movie-screen"><video ref={videoRef} src={film.src} playsInline preload="metadata" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onTimeUpdate={(event) => setElapsed(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration || 540)} onClick={togglePlayback}/>{!playing && <button className="big-play" onClick={togglePlayback} aria-label="Play movie">▶</button>}<div className="film-vignette"/></div></div>
        {mode !== 'Focus' && <div className="friend-rail" aria-label="Cinema participants">{friends.slice(0, mode === 'Social' ? 4 : 2).map((participant, index) => <div className={`camera-tile participant-tile participant-tile-${index}`} key={participant.uid}><div className={`avatar ${['art-coral', 'art-blue', 'art-gold', 'art-violet'][index]}`}>{participant.displayName?.slice(0, 2).toUpperCase() || '👤'}</div><div className="tile-name"><span className={participant.online ? 'voice-dot' : ''}/>{participant.displayName}{participant.online ? '' : ' · away'}</div><span className="tile-status">AVATAR · VIDEO IN NEXT PHASE</span></div>)}</div>}
        {!friends.length && <div className="empty-friends">The movie is ready. Invite a friend to take the other seat.</div>}
  ...
  if (screen === 'prejoin') return <main className="prejoin-page"><header className="simple-top"><button className="brand-mark" onClick={() => setScreen('home')}>◉ <span>social cinema</span></button><span className="step-note">YOU’RE ON THE LIST</span></header><section className="prejoin-card"><div className="preview-camera"><div className="preview-person"><div className="avatar art-violet">{getInitials(user)}</div><span>Camera previews arrive in the next phase</span></div><span className="preview-status">● CAMERA {cam ? 'ON' : 'OFF'} · MIC {mic ? 'ON' : 'OFF'}</span></div><div className="eyebrow">PULL UP A SEAT</div><h1>{room?.name || roomName}</h1><p>Hosted by <b>{room?.hostName || participants.find((participant) => participant.role === 'host')?.displayName || 'your friend'}</b> <span>·</span> <b>{room?.participantCount || 1} / {room?.maxParticipants || maxPeople} seats filled</b></p><div className="prejoin-toggles"><button onClick={() => setCam(!cam)}>{cam ? '▣' : '▧'} Camera <b>{cam ? 'On' : 'Off'}</b></button><button onClick={() => setMic(!mic)}>{mic ? '♫' : '♩'} Microphone <b>{mic ? 'On' : 'Off'}</b></button></div>{roomError && <div className="form-error" role="alert">{roomError}</div>}<button className="create-submit" disabled={roomBusy} onClick={handleJoinRoom}>{roomBusy ? 'Joining cinema…' : 'Join cinema'} <span>→</span></button><button className="skip-link" onClick={() => { setCam(false); setMic(false); handleJoinRoom() }}>Join without camera or mic</button></section>{toast && <div className="toast-note">{toast}</div>}</main>
